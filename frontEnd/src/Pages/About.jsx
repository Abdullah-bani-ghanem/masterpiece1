  ...
  return (
    <>
      ...
      <a
        ...
        className="fixed bottom-20 right-4 sm:right-6 z-10 w-12 sm:w-14 h-12 sm:h-14 rounded-full bg-green-500 flex items-center justify-center shadow-lg hover:bg-green-600 transition-colors"
      >
        <svg className="h-6 sm:h-8 w-6 sm:w-8 text-white" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      {showBackToTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-6 right-6 z-10 w-12 h-12 rounded-full bg-gray-800 dark:bg-gray-600 flex items-center justify-center shadow-lg hover:bg-yellow-600 transition-colors"
      ...
      {/* {showBackToTop && (
        <button 
          onClick={scrollToTop}
          className="fixed bottom-6 right-4 sm:right-6 z-10 w-10 sm:w-12 h-10 sm:h-12 rounded-full bg-gray-800 dark:bg-gray-600 flex items-center justify-center shadow-lg hover:bg-green-600 transition-colors"
          aria-label="Back to top"
        >
          <ArrowUp className="h-5 sm:h-6 w-5 sm:w-6 text-white" />
        </button>
      )} */}
      {/* 
      Chatbot Button
      <button
        ...
        className="fixed bottom-20 sm:bottom-24 left-4 sm:left-6 bg-green-600 text-white p-3 sm:p-4 rounded-full shadow-lg hover:bg-green-700 transition duration-300 z-40"
      >
        {showChatbot ? <FaTimes size={20} className="sm:w-6 sm:h-6" /> : <FaComments size={20} className="sm:w-6 sm:h-6" />}
      </button> */}

      {/* Chatbot Panel */}
      {/* {showChatbot && (
        <div className="fixed bottom-36 sm:bottom-40 left-4 sm:left-6 w-72 sm:w-80 bg-gray-800 rounded-lg shadow-xl overflow-hidden z-40 animate-slide-up">
          <div className="bg-green-600 p-3 sm:p-4">
            <h3 className="font-bold text-base sm:text-lg">Classic Car Concierge</h3>
            <p className="text-xs sm:text-sm text-amber-100">Ask us anything about our collection</p>
          </div>
         
          <div className="h-64 sm:h-80 overflow-y-auto p-3 sm:p-4" style={{scrollBehavior: 'smooth'}}>
            {chatHistory.map((chat, index) => (
              <div
                key={index}
                className={`mb-2 sm:mb-3 ${chat.sender === 'user' ? 'text-right' : 'text-left'}`}
              >
                <div
          ...
          <form onSubmit={handleChatSubmit} className="p-2 sm:p-3 border-t border-gray-700 flex">
            <input
              type="text"
              value={chatMessage}
              onChange={(e) => setChatMessage(e.target.value)}
              placeholder="Type your message..."
              className="flex-1 bg-gray-700 rounded-l p-2 outline-none text-sm sm:text-base"
            />
            <button
              type="submit"
              className="bg-green-600 text-white px-3 sm:px-4 rounded-r text-sm sm:text-base"
            >
              Send
            </button>
          </form>
